...
class DebitCreditCardsJSPage {
  ...
  constructor(page: Page) {
    ...
    this.cardNumberElement = this.cardElementForm.locator('#card-number-element');
    this.cardExpirationDateElement = this.cardElementForm.locator('#card-expiration-date-element');
  ...
  async elementsAllInOneReady() {
    await expect(
      await this.cardElement.frameLocator('iframe').locator('#cardNumber').innerHTML(),
    ).toBeDefined();
  ...
  async elementsIndividualReady() {
    ...
    await expect(await this.cardVerificationCodeElement.innerHTML()).toBeDefined();
  ...
  async fillCardElementsInput(input: {
    ...
    await this.cardNumberElement.frameLocator('iframe').locator('#cardNumber').fill(input.number);
    ...
    await this.cardVerificationCodeElement.frameLocator('iframe').locator('#cvc').fill(input.cvc);
  }
...
class DebitCreditCardsReactPage {
  ...
  constructor(page: Page) {
    ...
    this.cardExpirationDateElement = page.locator('#react-card-expiration-date-element');
    this.cardVerificationCodeElement = page.locator('#react-card-verification-code-element');
    this.successModal = page.getByTestId('capture-modal');
  }
